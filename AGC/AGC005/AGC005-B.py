# https://atcoder.jp/contests/agc005/submissions/18107677
# B - Minimum Sum
import sys
...
def resolve():
    ...
    B = [[a, idx + 1] for idx, a in enumerate(A)]
    B.sort()
    ...
    for i in range(n):
        a, idx = B[i]
        left = avl.search_lower(idx, 0)
        right = avl.search_higher(idx, n + 1)
        res += a * (idx - left) * (right - idx)
        avl.insert(idx)
    print(res)
