# https://atcoder.jp/contests/abc140/submissions/18837515
# E - Second Sum
import sys
...
def resolve():
    ...
    A = [[p, i + 1] for i, p in enumerate(P)]
    A.sort(reverse=True)

    avl = Avltree()
    res = 0
    for a, idx in A:
        l = avl.search_lower(idx, 0)
        r = avl.search_higher(idx, n + 1)
        if l != 0:
            ll = avl.search_lower(l, 0)
            res += a * (l - ll) * (r - idx)
        if r != n + 1:
            rr = avl.search_higher(r, n + 1)
            res += a * (idx - l) * (rr - r)
        avl.insert(idx)
    print(res)
